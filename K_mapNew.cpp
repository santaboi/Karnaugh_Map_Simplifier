...
struct minterm *new_ptr=new struct minterm[one_num];//比較完存現有值
    int index=0;
    for(int j=0;j<4;j++) 
    {
        for(int i=0;i<4;i++)
...
//**********************************queen macusky simplification************************************************************
    ...
    for(int i=0;i<one_num;i++)
    {   
        cout<<cur_ptr[i].binary[0]<<cur_ptr[i].binary[1]<<cur_ptr[i].binary[2]<<cur_ptr[i].binary[3]<<' ';
    }
    cout<<endl;
    for(int i=0;i<new_ind;i++)
    {   cout<<new_ptr[i].binary[0]<<new_ptr[i].binary[1]<<new_ptr[i].binary[2]<<new_ptr[i].binary[3]<<' ';}
    cout<<endl;
    //一圈完成
